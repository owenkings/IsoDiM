...
class ModelType(enum.Enum):
    ...
    DATA = enum.auto()  # the model predicts x_1 (clean data) - x-prediction
...
class Transport:
    ...
    def check_interval(
        ...
        *, 
        ...
        if (type(self.path_sampler) in [path.VPCPlan]):
            ...
            # DATA (x-prediction) is also stable like VELOCITY
    ...
    def training_losses(
        ...
        if self.model_type == ModelType.VELOCITY:
            ...
            # X-prediction: directly predict the clean data x1
            # This is better suited for bounded FSQ space [-1, 1]
    ...
    def get_drift(
        ...
        def data_ode(x, t, model, **model_kwargs):
            """X-prediction ODE: convert predicted x1 to velocity
            For linear path: x_t = (1-t) * x_0 + t * x_1
            So velocity v = dx/dt = x_1 - x_0 = (x_1 - x_t + t * x_1) / (1-t+eps)
            Simplified: v = (x_1 - x_t) / (1 - t + eps) where eps prevents division by zero
            """
            x1_pred = model(x, t, **model_kwargs)
            t_expand = path.expand_t_like_x(t, x)
            # Convert predicted x1 to velocity: v = (x1 - xt) / (1 - t)
            # Add small epsilon for numerical stability near t=1
            eps = 1e-5
            velocity = (x1_pred - x) / (1 - t_expand + eps)
            return velocity
    ...
    def get_score(
        ...
        if self.model_type == ModelType.NOISE:
            ...
            # Convert predicted x1 to score: first get velocity, then score
            def data_score_fn(x, t, model, **kwargs):
                x1_pred = model(x, t, **kwargs)
                t_expand = path.expand_t_like_x(t, x)
                eps = 1e-5
                velocity = (x1_pred - x) / (1 - t_expand + eps)
                return self.path_sampler.get_score_from_velocity(velocity, x, t)
            score_fn = data_score_fn
        else:
            raise NotImplementedError()
