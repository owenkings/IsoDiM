"""
IsoDiM - Dataset Classes
========================
PyTorch Dataset classes for motion data loading.
"""
...
def collate_fn(batch):
    """Custom collate function that sorts by sequence length."""
...
class AEDataset(data.Dataset):
    """
    Dataset for autoencoder (tokenizer) training.
    
    Loads motion snippets of fixed window size for reconstruction training.
    """
    ...
    def __init__(self, mean, std, motion_dir, window_size, split_file):
        ...
        for name in tqdm(id_list, desc="Loading motion data"):
            try:
        ...
        self.mean = mean
        self.std = std
        print(f"Total motions: {len(self.data)}, snippets: {self.cumsum[-1]}")
...
class Text2MotionDataset(data.Dataset):
    """
    Dataset for text-to-motion training and evaluation.
    
    Loads motion sequences paired with text descriptions.
    """
    ...
    def __init__(
        ...
        for name in tqdm(id_list, desc="Loading text-motion pairs"):
            try:
                motion = np.load(pjoin(motion_dir, name + '.npy'))
